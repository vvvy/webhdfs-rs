...
# During preparation, SHA-512 checksums are calculated for each of r: chunks set up by PROGRAM.
# The program under test is expected to execute the PROGRAM below (actually, POUT) by doing seeks and reads as requested.
# Upon each read, the program reads from HDFS then writes the content read to a file specified by 3rd part of POUT item.
#
# itt.sh --prepare uploads the testfile to hdfs, calculates checksums and other necessary data.
#
# itt.sh --validate validates checksums of the files generated by the program being tested, against the reference checksums generated
...
prepare() {
    ...
    if [ ! -f $TESTDATA_DIR/.prepared ] ; then 
        create-source &&
        create-shasums &&
        create-natmap &&
        create-args &&
